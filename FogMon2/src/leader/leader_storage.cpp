...
//Changed from J
void LeaderStorage::addGossipMsg(const std::string& sender,const std::string& receiver) {
    char *zErrMsg = 0;
    std::stringstream query;
    query << "INSERT INTO Test (sender, receiver, timestamp, isLeader) VALUES (\"" << sender << "\", \"" << receiver << "\", DATETIME('now'), 'True')";
    int err = sqlite3_exec(this->db, query.str().c_str(), 0, 0, &zErrMsg);
    isError(err, zErrMsg, "addGossipMsg");
}
