...
public:
    ...
    std::vector<Report::report_result> getReport(bool complete);
    Report::report_result getReport(Message::node node, bool complete);

    //Changed from J
    void addGossipMsg(const std::string& sender, const std::string& receiver);
    ...
    virtual std::vector<Message::node> removeOldLNodes(int seconds, int &leaders_num, bool force=false);
    ...
    virtual std::vector<Report::test_result> getLatency(Message::node node, bool complete);
    virtual std::vector<Report::test_result> getBandwidth(Message::node node, bool complete);
};
