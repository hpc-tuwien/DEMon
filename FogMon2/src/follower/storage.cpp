...
void Storage::createTables() {
    ...
    vector<string> query = {"CREATE TABLE IF NOT EXISTS Hardware (time TIMESTAMP PRIMARY KEY, cores INTEGER, free_cpu REAL, memory INTEGER, free_memory REAL, disk INTEGER, free_disk REAL)",
                            ...
                            "CREATE TABLE IF NOT EXISTS Test (id INTEGER PRIMARY KEY AUTOINCREMENT, sender STRING, receiver STRING, timestamp TIMESTAMP, isLeader BOOLEAN)",
...
//Changed from J
void Storage::addFollowerMsgToLeader(const std::string& sender,const std::string& receiver) {
    char *zErrMsg = 0;
    std::stringstream query;
    query << "INSERT INTO Test (sender, receiver, timestamp, isLeader) VALUES (\"" << sender << "\", \"" << receiver << "\", DATETIME('now'), 'False')";
    int err = sqlite3_exec(this->db, query.str().c_str(), 0, 0, &zErrMsg);
    isError(err, zErrMsg, "addFollowerMsgToLeader");
}
