...
bool Follower::checkServer(vector<Message::node> mNodes) {
    ...
    if( curr > min + 5) {
        ...
        if(abs(meanCurr-mean)/meanCurr > 0.1) {
            return true;
        }
    } 
...
void Follower::timer() {
    ...
    while(this->running) {
        ...
        //Changed from J 
        if(this->myNode.id != this->nodeS.id){
            this->getStorage()->addFollowerMsgToLeader(this->myNode.id, this->nodeS.id);
        }
...
float Follower::testBandwidth(Message::node node, float old, int &state) {
    float result = -1;
    int port;
    string myIp;
    switch(state) {
        case 0: //base state
            port = this->connections->sendStartIperfTest(node);
            if(port != -1) {
                for(int i=0; i<3; i++) {
                    result = this->testBandwidthIperf(node.ip,port);
                    if (result >= 0)
                        break;
                    usleep(10000);
                }
        ...
        case 1: //a test is already done
            port = this->connections->sendStartEstimateTest(node, myIp);
            if(port != -1) {
                result = this->testBandwidthEstimate(node.ip, myIp, old);
    ...
    if(port == -1 || result <0) {
        cout << "Test ["<< node.ip <<"]: " << port << " " << result << endl;
    }
